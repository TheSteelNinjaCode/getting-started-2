...
class Post implements IModel
{
    ...
    protected function includeUser(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        ...
        if (!empty($foreignKeyIds)) {
            $tableModelName = new User($this->pdo);
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Post.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Post.");
            }
        }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Post model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if (empty($conditions)) {
            throw new \Exception("No valid where provided for finding a unique record in Post.");
        }
        ...
        foreach ($includes as $relation => $include) {
            if ($include) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Post model.");
                }
            }
        }
        ...
        if ($format === 'object') {
            return new Post($this->pdo, $record);
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Post.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Post.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Post Model.");
            }
        }
            ...
            if (!empty($where)) {
                ...
                foreach ($where as $fieldName => $fieldValue) {
                    if (array_key_exists($fieldName, $this->fields)) {
                        ...
                        throw new \Exception("The '$fieldName' field does not exist in the Post model.");
                    }
                }
    ...
    public function upsert(array $data, string $format = 'array'): array | object
    {
        ...
        Utility::checkForInvalidKeys($data, $acceptedCriteria, 'Post');
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Post.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Post.");
        }
