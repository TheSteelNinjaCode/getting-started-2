...
class User implements IModel
{
    ...
    protected function includePost(array $items, array $selectedFields = [], array $includeSelectedFields = []) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $tableName = $dbType == 'pgsql' ? "\"Posts\"" : "`Posts`";
        $modelName = 'Post';
        $relationName = 'Post';
    ...
    protected function connectPost(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->dbType;
        $quotedTableName = $dbType == 'pgsql' ? "\"Users\"" : "`Users`";
        $tableName = $dbType == 'pgsql' ? "\"Posts\"" : "`Posts`";
        ...
        $relationModel = new Post($this->pdo);
        ...
        if (!empty($foundUnique) && $connectType === 'connect') {
            try {
                ...
                throw new \Exception("Error connecting Post: " . $e->getMessage());
            }
        }
        ...
        if (!empty($foundUnique) && $connectType === 'disconnect') {
            try {
                ...
                throw new \Exception("Error disconnecting Post: " . $e->getMessage());
            }
        }
    }
    ...
    public function findUnique(array $criteria, string $format = 'array'): array | object
    {
        ...
        if (empty($conditions)) {
            throw new \Exception("No valid where provided for finding a unique record in User.");
        }
    ...
    public function update(array $data, string $format = 'array'): array | object
    {
        ...
        try {
            ...
            if (!empty($relationNames)) {
                $lowercaseRelationNames = array_map('strtolower', $relationNames);
                foreach ($data as $relationName => $relationDataName) {

                    $targetStringLower = strtolower($relationName);
                    if (in_array($targetStringLower, $lowercaseRelationNames)) {
                        ...
                        if (isset($relationDataName['create']) && is_array($relationDataName['create'])) {
                            ...
                            if ($checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("To create a new record, the value of 'create' must be a single array with names as keys and the data to create as values in Post model. example: ['create' => ['name' => 'someName']]");
                            }
                            ...
                            if ($checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("To create many records, use an associative array with the field names as keys and the data to create as values in Post model. ['createMany' => [['name' => 'someName'], ['name' => 'someOtherName']]");
                            }
                    ...
                    if (isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate']) || isset($relationDataName['disconnect']) || isset($relationDataName['update']) || isset($relationDataName['updateMany'])) {
                        ...
                        if (isset($relationDataName['connect'])) {
                            ...
                            if (isset($relationDataName['connect']) && $checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("The 'connect' key must be an associative array with the field names as keys and the data to create as values related Post model. example: ['connect' => ['id' => 'someId']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['connectOrCreate'])) {
                            ...
                            if (isset($relationDataName['connectOrCreate']) && $checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("The 'connectOrCreate' key must be an associative array with the field names as keys and the data to create as values related Post model. example: ['connectOrCreate' => ['where' => ['id' => 'someId'], 'create' => ['name' => 'someName']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['disconnect'])) {
                            ...
                            if (!is_bool($disconnectData)) {
                                ...
                                if (isset($relationDataName['disconnect']) && $checkArrayContentType !== ArrayType::Value) {
                                    throw new \Exception("The 'disconnect' key must be an associative array with the field names as keys and the data to create as values related Post model. example: ['disconnect' => ['id' => 'someId']]");
                                }
                            }
                        }
